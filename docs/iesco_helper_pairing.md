...
4. Later, click **Fetch** for one meter or **Fetch All** without reconnecting or downloading again. The progress modal shows processed, updated, archived-PDF, and failed counts. For each bill, the helper fetches the actual PITC page, prints that page to PDF with the installed Microsoft Edge browser, sends the parsed bill and PDF to TMS, and exits. Existing archived PDFs are never overwritten.
...
The build computer needs the repository, Python virtual environment, requests, BeautifulSoup, and PyInstaller. The EXE contains the Python runtime and bill parser; staff computers need Microsoft Edge, which is used in headless mode to print the actual PITC bill page. The helper uses the HTTPS base URL embedded at build time, and never accepts a server URL from a browser link.
...
5. Confirm the download is available to an authorised user. On a Pakistani Windows computer with Microsoft Edge installed, install, pair, fetch one active test reference, and confirm the saved reading is **Parsed**, its **View PDF** button opens the actual PITC bill, and no invoice changed.
