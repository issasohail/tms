...
from leases.services.police_verification import (
    ...
    create_tenant_registration_submission,
...
def public_lease_ledger(request, token):
    link = _get_valid_whatsapp_link(token, WhatsAppExternalLinkToken.LINK_LEDGER_VIEW)
        ...
        transactions.append(
            {
                ...
                "type": "Payment",
                "description": payment.reference_number or f"Payment #{payment.pk}",
                "amount": amount,
            }
        )
    transactions.sort(
    ...
    return render(
        request,
        "leases/public_lease_ledger.html",
        {
            "lease": lease,
            "tenant": lease.tenant,
...
def resolve_placeholders(lease, text):
    ...
    replacements = {
        ...
        if lease.monthly_rent is not None
        else "",
        ...
        if lease.society_maintenance is not None
        else "",
...
def prepare_transaction_columns(transactions):
    ...
    if len(transactions) <= 25:
        ...
        columns.append([{"index": i + 1, **t} for i, t in enumerate(transactions)])
    else:
        ...
        for i in range(0, len(transactions), items_per_column):
            ...
            columns.append(
                [{"index": j + 1, **t} for j, t in enumerate(column, start=i)]
            )
    return columns
...
class LeaseListView(SingleTableView):
    ...
    def _annotate_list_financials(self, queryset):
    ...
    def _attach_list_balances(self, leases):
        ...
        sd_by_lease = defaultdict(lambda: defaultdict(lambda: Decimal("0.00")))
        for row in sd_rows:
            sd_by_lease[row["lease_id"]][row["type"]] = row["total"] or Decimal("0.00")

        for lease in leases:
            invoice_total = invoice_totals.get(lease.id, Decimal("0.00"))
            payment_total = payment_totals.get(lease.id, Decimal("0.00"))

            lease.list_balance = invoice_total - payment_total
    ...
    def get_context_data(self, **kwargs):
        ...
        if table is not None and hasattr(table, "page"):
            page_leases = [row.record for row in table.page.object_list]
        else:
        ...
        context["all_properties"] = Property.objects.only("id", "property_name")
        ...
        if property_id:
            ...
            ).only("id", "property_id", "unit_number")
        else:
...
class LeaseTenantOrderMixin:
    def get_form(self, form_class=None):
        ...
        if {"first_name", "last_name"} <= fields:
            qs = Tenant.objects.order_by(Lower("first_name"), Lower("last_name"), "id")
        elif "name" in fields:
...
def _tenant_age(tenant):
    dob = getattr(tenant, "date_of_birth", None)
    if not dob:
        return ""
...
def _tenant_family_options():
    tenants = Tenant.objects.annotate(
        full_name=Concat("first_name", Value(" "), "last_name")
    ).order_by(Lower("full_name"), "id")
    options = []
    for tenant in tenants:
        phone = tenant.phone or tenant.phone2 or tenant.phone3 or ""
        age = _tenant_age(tenant)
        label_bits = [tenant.get_full_name()]
        if age != "":
...
def _save_tenant_document_uploads(request, lease):
    tenant = getattr(lease, "tenant", None)
    if not tenant:
        return
    field_map = {
        "tenant_photo": "photo",
        "tenant_cnic_front": "cnic_front",
        "tenant_cnic_back": "cnic_back",
    }
    changed = []
    for input_name, tenant_field in field_map.items():
        uploaded = request.FILES.get(input_name)
        if uploaded:
            setattr(tenant, tenant_field, uploaded)
            changed.append(tenant_field)
    if changed:
...
class LeaseCreateView(LoginRequiredMixin, LeaseTenantOrderMixin, CreateView):
    ...
    def _handle_quick_add(self, lease):
        ...
        if created_links:
            messages.success(self.request, f"Added {created_links} family member(s).")
...
@login_required
@require_POST
def lease_family_create_and_add(request, pk):
    lease = get_object_or_404(Lease, pk=pk)
    ...
    with transaction.atomic():
        ...
        link, created_link = LeaseFamilyMember.objects.get_or_create(
            lease=lease,
            primary_tenant=lease.tenant,
            family_member=tenant,
        ...
        if not created_link:
            ...
            relationship = relationship_defaults.get("relationship")
            if relationship_type and link.relationship_type_id != relationship_type.pk:
                link.relationship_type = relationship_type
...
@require_http_methods(["GET", "POST"])
def public_police_verification(request, token):
    lease, link = get_valid_police_link(token)
    if lease is None:
        return render(
            request,
            "leases/public_link_status.html",
            {
                "title": "Police Verification Link Expired",
                "message": "This police verification link is invalid or expired. Please request a new link from the office.",
            },
            status=403,
        )
    ...
    if tenant_data or any(
        ...
        saved_messages.append("Tenant information sent for staff approval.")

    police_file = request.FILES.get("police_file")
    if police_file:
        create_pending_police_submission(
            lease,
            police_file,
            PendingPoliceVerificationSubmission.SOURCE_PUBLIC_LINK,
            phone=getattr(lease.tenant, "phone", "") or "",
            notes=(request.POST.get("police_notes") or "").strip(),
        )
        saved_messages.append("Police verification file sent for staff approval.")

    family_total_raw = request.POST.get("family-TOTAL_FORMS") or "0"
    try:
        family_total = max(0, min(int(family_total_raw), 10))
    except (TypeError, ValueError):
        family_total = 0
    family_rows = []
    for index in range(family_total):
        full_name = (request.POST.get(f"family-{index}-full_name") or "").strip()
        cnic = (request.POST.get(f"family-{index}-cnic") or "").strip()
        relation = (request.POST.get(f"family-{index}-relation") or "").strip()
        phone = (request.POST.get(f"family-{index}-phone") or "").strip()
        ...
        if not first_name:
            errors.append(f"Family member {index + 1}: name is required.")
        if not relation:
            errors.append(f"Family member {index + 1}: relationship is required.")
        parsed_dob = None
        if dob:
            try:
                parsed_dob = date.fromisoformat(dob)
...
def lease_police_verification_link(request, pk):
    ...
    phone = getattr(lease.tenant, "phone", "") or ""
    use_api = request.POST.get("use_api") == "1"
    api_results = []
    if use_api and phone:
        ...
        pdf_result = service.send_pdf_bytes(
            phone,
            pdf_bytes,
            filename=f"police-verification-lease-{lease.pk}.pdf",
            caption="Police verification information PDF.",
            tenant=lease.tenant,
            lease=lease,
        )
...
def public_lease_family_cnic_check(request, token):
    ...
    exists = Tenant.objects.filter(cnic_digits=cnic_digits).exists()
    pending_exists = PendingLeaseFamilyMemberSubmission.objects.filter(
        cnic_digits=cnic_digits,
        status=PendingLeaseFamilyMemberSubmission.STATUS_PENDING,
    ).exists()
...
def approve_pending_family_submission(pending, user):
    if pending.status != PendingLeaseFamilyMemberSubmission.STATUS_PENDING:
    ...
    if existing_tenant:
        already_linked = LeaseFamilyMember.objects.filter(
            lease=pending.lease,
    ...
    with transaction.atomic():
        ...
        link, _created = LeaseFamilyMember.objects.get_or_create(
            lease=pending.lease,
            primary_tenant=pending.primary_tenant,
            family_member=tenant,
            defaults={
        ...
        pending.created_tenant = tenant
        pending.created_family_member = link
        pending.status = PendingLeaseFamilyMemberSubmission.STATUS_APPROVED
        pending.reviewed_by = user
        pending.reviewed_at = timezone.now()
        pending.save(
...
class LeaseUpdateView(LoginRequiredMixin, LeaseTenantOrderMixin, UpdateView):
    ...
    def get_queryset(self):
        return super().get_queryset().select_related("tenant", "unit", "unit__property")
    ...
    def form_valid(self, form):
        ...
        if not family_fs.is_valid():
            ...
            ctx["family_formset"] = family_fs
            return self.render_to_response(ctx)
        if not renewal_fs.is_valid():
            messages.error(
                self.request, "Renewal history has errors. Please fix and try again."
            )
            ctx = self.get_context_data(form=form)
            ctx["family_formset"] = family_fs
        ...
        if debug_sql:
            ...
            sql = _run_sql(
                lambda: update_billing_on_change(
                    self.object,
                    old,
...
class GetUnitsView(View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if property_id:
            units = Unit.objects.filter(property_id=property_id).values(
                "id", "unit_number"
            )
...
class PrintLeaseView(View):
    def get(self, request, pk):
        ...
        html_string = render_to_string(
            "leases/lease_pdf_detail.html",
            {"lease": lease, "static_url": "/static/", "media_url": "/media/"},
        )
...
class LeaseDetailView(LoginRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        lease_total_payment = (
            ...
            + (lease.society_maintenance or ZERO)
            + (lease.water_charges or ZERO)
            + (lease.internet_charges or ZERO)
        )
...
def lease_balance(request, pk):
    lease = get_object_or_404(Lease, pk=pk)
    return Response({"balance_due": lease.get_balance_due()})
...
def tenant_quick_search(request):
    ...
    if not q and want_all:
        ...
        qs = Tenant.objects.filter(filters).order_by("first_name", "last_name", "id")[
            :50
        ]
...
def get_lease_info(request):
    ...
    try:
        lease = Lease.objects.get(tenant_id=tenant_id, unit_id=unit_id, status="active")
...
class RenewLeaseView(LoginRequiredMixin, FormView):
    ...
    def form_valid(self, form):
        ...
        renewed_lease = lease.renew_lease(
            ...
            rent_increase_percent=float(form.cleaned_data["rent_increase_percent"]),
...
class LeaseLedgerView(LoginRequiredMixin, TemplateView):
    ...
    def _build_filter_context(self):
        ...
        if not show_inactive:
            leases_qs = leases_qs.filter(start_date__lte=today, end_date__gte=today)
        ...
        for l in lease_candidates:
            ...
            unit_str = (
                f"{l.unit.property.property_name}-{l.unit.unit_number}"
    ...
    def get_context_data(self, **kwargs):
        ...
        property_only = bool(
            fc["current_property"]
            and not fc["current_unit"]
            and not fc["current_lease"]
        )
        ...
        ctx.update(
            {
                **fc,
                ...
                "total_paid": sum(
                    t["amount"]
                    for t in transactions
                    if t["type"] == "Payment" and t["amount"] > 0
                ),
                "total_owed": sum(
...
def send_ledger_email(request, lease_id):
    ...
    try:
        ...
        # Attach PDF
        email.attach(f"lease_{lease.id}_ledger.pdf", pdf_content, "application/pdf")
    ...
    try:
        ...
        if lease.notes or lease.security_deposit_return_notes:
            ...
            if lease.security_deposit_return_notes:
                notes.append(f"Deposit Notes: {lease.security_deposit_return_notes}")
                ...
                return JsonResponse(
                    {
                        "success": True,
                        "message": msg,
                        "docx_url": lease.generated_agreement_docx.url
                        if lease.generated_agreement_docx
                        else None,
                        "pdf_url": lease.generated_agreement_pdf.url
...
class SendAgreementEmailView(View):
    def post(self, request, pk):
        ...
        try:
            ...
            if success:
                messages.success(request, "Agreement sent via email successfully!")
            else:
...
def generate_lease_agreement(request, lease_id):
    ...
    response = HttpResponse(pdf, content_type="application/pdf")
    response["Content-Disposition"] = (
        f'attachment; filename="lease_agreement_{lease_id}.pdf"'
    )
    return response
...
def create_relationship_type_ajax(request):
    ...
    if not name:
        return JsonResponse({"ok": False, "message": "Relationship name is required."}, status=400)

    existing = LeaseRelationshipType.objects.filter(name__iexact=name).first()
    if existing:
        if not existing.is_active:
            existing.is_active = True
            existing.save(update_fields=["is_active"] )
        return JsonResponse({"ok": True, "id": existing.pk, "text": existing.name, "created": False})

    base_code = slugify(name)[:70] or "relationship"
    code = base_code
    counter = 2
    ...
    relationship = LeaseRelationshipType.objects.create(
    ...
    if (
        ...
        for key, value in request.POST.items():
            if key.startswith("clause_"):
                ...
                try:
                    clause = history.clauses.get(id=clause_id)
                    clause.template_text = value
                    clause.is_customized = True
                    clause.save(update_fields=["template_text", "is_customized"])
                except Exception:
                    continue

        return JsonResponse(
            {"status": "success", "message": "Clauses updated successfully"}
        )

    # Selected-history PDF/DOCX files are generated on demand by download views.
    # Keep the old master-lease auto-generation path disabled here to avoid
    # writing a stale master PDF while editing an older history.
    if False and (
        not lease.generated_agreement_pdf
        or not os.path.exists(lease.generated_agreement_pdf.path)
    ):
        context = {
            "lease": lease,
        ...
        with tempfile.NamedTemporaryFile(delete=True, suffix=".pdf") as output:
            ...
            cnic = lease.tenant.cnic.replace("-", "") if lease.tenant.cnic else "nocnic"
...
def download_preview_pdf(request, lease_id):
    ...
    filename = (
        ...
        f"{lease.tenant.cnic}-{lease.id}-history-{history.renewal_number}-agreement_not_sign.pdf"
    )
...
class SecurityDepositListView(LeaseSecurityMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx["sec_totals"] = (
            security_deposit_totals(self.lease)
            if self.lease
            else {
                "required": 0,
                "paid_in": 0,
...
from core.models import GlobalSettings
from leases.whatsapp import build_whatsapp_url, render_whatsapp_template

from .forms import (
    AgreementPlaceholderForm,
    DefaultClauseForm,
    LeaseClauseFormSet,
    WhatsAppTemplateForm,
)
from .models import AgreementPlaceholder, Lease, WhatsAppTemplate
...
def default_clause_create(request):
    ...
    return render(
        request,
        "leases/default_clause_form.html",
        {
            "form": form,
...
@login_required
@permission_required("leases.add_agreementplaceholder", raise_exception=True)
def agreement_placeholder_create(request):
    if request.method == "POST":
        form = AgreementPlaceholderForm(request.POST)
        if form.is_valid():
            form.save()
    ...
    return render(
        request,
        "leases/agreement_placeholder_form.html",
        {
            "form": form,
...
@login_required
@permission_required("leases.change_agreementplaceholder", raise_exception=True)
def agreement_placeholder_edit(request, pk):
    placeholder = get_object_or_404(AgreementPlaceholder, pk=pk)
    if request.method == "POST":
        form = AgreementPlaceholderForm(request.POST, instance=placeholder)
        if form.is_valid():
            form.save()
...
@login_required
def lease_welcome_whatsapp(request, pk):
    lease = get_object_or_404(
        Lease.objects.select_related("tenant", "unit", "unit__property"),
        pk=pk,
    )
    settings_obj = GlobalSettings.get_solo()
    template, rendered_message = render_whatsapp_template(
    ...
    return render(
        request,
        "leases/lease_whatsapp_preview.html",
        {
            "lease": lease,
            "template": template,
            "template_type": WhatsAppTemplate.TEMPLATE_TENANT_WELCOME,
            "title": "Tenant Welcome WhatsApp",
            "phone": phone,
...
def _preview_whatsapp_template(body):
    rendered = body or ""
    placeholders = _active_agreement_placeholders()
    for placeholder in placeholders:
        value = placeholder.default_value or placeholder.label or placeholder.key
        rendered = rendered.replace(f"[{placeholder.key}]", value)
    return rendered


@login_required
@permission_required("leases.view_whatsapptemplate", raise_exception=True)
def whatsapp_template_list(request):
    templates = WhatsAppTemplate.objects.order_by("template_type")
    return render(
        request,
        "leases/whatsapp_template_list.html",
        {
            "templates": templates,
        },
    )
...
@login_required
@permission_required("leases.add_whatsapptemplate", raise_exception=True)
def whatsapp_template_create(request):
    if request.method == "POST":
        form = WhatsAppTemplateForm(request.POST)
        if form.is_valid():
            form.save()
            messages.success(request, "WhatsApp template added.")
            return redirect("leases:whatsapp_template_list")
    else:
        form = WhatsAppTemplateForm()

    return render(
        request,
        "leases/whatsapp_template_form.html",
        {
            "form": form,
            "title": "Add WhatsApp Template",
            "placeholders": _active_agreement_placeholders(),
            "preview_text": _preview_whatsapp_template(request.POST.get("body", "")),
        },
    )


@login_required
@permission_required("leases.change_whatsapptemplate", raise_exception=True)
def whatsapp_template_edit(request, pk):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            form.save()
            messages.success(request, "WhatsApp template updated.")
            return redirect("leases:whatsapp_template_list")
    else:
        form = WhatsAppTemplateForm(instance=template)

    preview_body = request.POST.get("body", template.body)
    return render(
        request,
        "leases/whatsapp_template_form.html",
        {
            ...
            "placeholders": _active_agreement_placeholders(),
            "preview_text": _preview_whatsapp_template(preview_body),
        },
    )
...
def _new_p(doc, align=WD_ALIGN_PARAGRAPH.JUSTIFY):
    p = doc.add_paragraph()
    p.alignment = align
...
def add_signature_block(doc, lease, history=None):
    ...
    def witness_lines(label, person):
        return [
            f"{label}: {person.get_full_name() if person else blank_party_value}",
            f"CNIC: {format_cnic(person.cnic) if person and person.cnic else blank_party_value}",
            f"Phone: {format_phone(person.phone) if person and person.phone else blank_party_value}",
        ]

    set_cell(tw.cell(0, 0), ["_________________________"])
    set_cell(tw.cell(0, 1), ["_________________________"])
    set_cell(tw.cell(1, 0), witness_lines("Witness 1", witness1), bold_prefix=True)
    set_cell(tw.cell(1, 1), witness_lines("Witness 2", witness2), bold_prefix=True)
...
def html_to_docx_bytes(html: str, lease, history=None) -> bytes:
    ...
    for clause_div in container.select(".clauses-section > .clause"):
        ...
        initial_run = p.add_run("________")
        initial_run.font.name = "Times New Roman"
        p.add_run("  ")
...
def reset_clauses_from_default(request, pk):
    ...
    if not default_clauses.exists():
        ...
        return redirect(
            f"{reverse('leases:edit_clauses', kwargs={'pk': pk})}?history={history.pk}"
        )

    # OPTION A: overwrite everything
    history.clauses.all().delete()
    for default_clause in default_clauses:
        LeaseRenewalClause.objects.create(
            renewal=history,
            clause_number=default_clause.clause_number,
            template_text=default_clause.body,
            is_customized=False,
        )

    messages.success(request, "Clauses reset to active default clauses.")
    return redirect(
        f"{reverse('leases:edit_clauses', kwargs={'pk': pk})}?history={history.pk}"
    )
