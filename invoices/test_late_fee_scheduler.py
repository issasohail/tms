...
class LateFeeSchedulerTests(TestCase):
    ...
    def test_exact_schedule_no_early_and_missed_manual_catchup(self, _send):
        ...
        self.assertEqual(
            list(
                self.invoice.late_fee_reminders.order_by("reminder_number").values_list(
                    "reminder_number", flat=True
                )
            ),
            [1, 2],
        )
    ...
    def test_scheduler_manual_scheduler_is_idempotent(self, _send):
        ...
        self.assertEqual(
            self.invoice.items.filter(category__name="Late Fee").count(), 1
        )
    ...
    def test_pending_paid_maximum_and_dry_run(self, _send):
        ...
        settings_obj.save()
        dry = run_due_late_fee_reminders(
        ...
        self.assertEqual(
            run_due_late_fee_reminders(source="manual", today=date(2026, 9, 1))[
                "processed"
            ],
            0,
        )
    ...
    def test_zero_amount_invoice_is_never_sent_or_charged(self, send):
        ...
        result = run_due_late_fee_reminders(source="manual", today=date(2026, 8, 12))

        self.assertEqual(result["excluded_zero_amount"], 1)
        self.assertNotIn(
            zero_invoice.pk, [item["invoice_id"] for item in result["details"]]
        )
    ...
    @patch("invoices.late_fees._send_late_fee_whatsapp", return_value={"ok": True})
    def test_enabled_staff_summary_is_sent_after_processed_run(
        self, _tenant_send, staff_send
    ):
        settings_obj = GlobalSettings.get_solo()
    ...
    def test_control_screen_can_place_and_clear_invoice_hold(self):
        ...
        with patch(
            "invoices.views_late_fee.timezone.localdate",
            return_value=date(2026, 8, 16),
        ):
    ...
    def test_hold_also_blocks_approval_of_an_already_pending_fee(self):
        ...
        with patch(
            "invoices.late_fees.timezone.localdate", return_value=date(2026, 8, 16)
        ):
