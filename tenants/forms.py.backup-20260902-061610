            ...
            for prefix, label, always_required in (
                ...
                if always_required:
                    for field_label, value in zip(
                        ...
                        if not value:
                            role_errors.append(f"{label} {field_label} is required.")
                    if values[2]:
                        try:
                            validate_cnic(values[2])
                        except ValidationError:
                            role_errors.append(f"{label} CNIC must contain exactly 13 digits.")
                elif has_any_data and not phone:
