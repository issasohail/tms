...
def _request_tenant(request_obj):
    return getattr(getattr(request_obj, "lease", None), "tenant", None)
...
def _request_json(request, item):
    ...
    return {
        ...
        "building_id": item.building_id or "",
...
def _lease_label(lease):
    tenant_name = _tenant_name(lease.tenant) or str(lease.tenant)
    unit_label = str(lease.unit)
    end_date = lease.end_date.strftime("%Y-%m-%d") if lease.end_date else ""
    return f"{tenant_name} - {unit_label} - End: {end_date}"
...
def _request_from_share_token(token):
    ...
    return get_object_or_404(
        MaintenanceRequest.objects.select_related("building", "unit", "lease", "lease__tenant"),
        pk=request_id,
    ), ""
...
class MaintenanceRequestListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        qs = (
            ...
            .select_related("building", "unit", "lease", "lease__tenant", "assigned_to", "category_ref")
        ...
        if q:
            qs = qs.filter(
                ...
                | Q(lease__tenant__first_name__icontains=q)
                | Q(lease__tenant__last_name__icontains=q)
...
def category_request_list(request, pk):
    ...
    requests_qs = (
        ...
        .select_related("building", "unit", "lease", "lease__tenant")
        .prefetch_related("media")
        .annotate(active_media_count=Count("media", filter=Q(media__is_active=True)))
...
class MaintenanceRequestDetailView(LoginRequiredMixin, DetailView):
    ...
    def get_queryset(self):
        return MaintenanceRequest.objects.select_related(
            "building", "unit", "lease", "lease__tenant", "assigned_to", "created_by", "updated_by"
...
def request_inline_update(request, pk):
    ...
    if field not in {"title", "description", "status", "cost", "unit", "reported_date"}:
    ...
    item = MaintenanceRequest.objects.select_related(
        "building", "unit", "lease", "lease__tenant", "category_ref"
...
def request_whatsapp(request, pk):
    item = get_object_or_404(
        MaintenanceRequest.objects.select_related("building", "unit", "lease", "lease__tenant").prefetch_related("media"),
        pk=pk,
    )
