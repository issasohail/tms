...
class CNICAutoOrientationTests(SimpleTestCase):
    ...
    def test_front_number_retry_uses_a_wide_lower_card_crop(self):
        source = self._jpeg((1000, 600))

        encoded = _enhanced_front_image_data(source)

        with Image.open(BytesIO(base64.b64decode(encoded))) as image:
            self.assertEqual(image.size, (2280, 900))

    def test_front_number_retry_caps_large_camera_images(self):
        source = self._jpeg((3000, 1800))
...
class CNICRegistrationTemplateCoverageTests(SimpleTestCase):
    ...
    def test_extracted_portrait_bypasses_the_second_crop_dialog(self):
        self.assertIn("photoInput.dataset.tmsImageEditorApplying='1'", self.identity_source)

    def test_free_crop_starts_at_the_uploaded_image_aspect_ratio(self):
        self.assertIn(
            'state.ratio === "free" ? sourceWidth / sourceHeight',
            self.image_editor_source,
        )
    ...
    def test_superseded_ocr_request_always_releases_pending_count(self):
        self.assertIn(
            "if(request===seq){stopTimer();activePhase=''}\n          setSubmitPending(context.form,false);",
            self.identity_source,
        )

    def test_front_and_back_are_read_as_separate_ocr_phases(self):
        self.assertIn("body.append('ocr_phase',phase)", self.identity_source)
        self.assertIn("body.append('front_token',frontToken)", self.identity_source)
        self.assertIn("async function readFront()", self.identity_source)
        self.assertIn("async function readBack()", self.identity_source)
