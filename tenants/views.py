...
CNIC_STAGED_TOKEN_MAX_AGE = 60 * 60
CNIC_STAGED_TOKEN_SALT = "tenants.cnic-staged-front"
...
def _existing_tenant_ocr_payload(cnic):
    """Return a minimal, form-safe existing-tenant payload for a verified OCR CNIC."""
    from tenants.models import normalize_cnic

    digits = normalize_cnic(cnic)
    if not digits:
        return None
    tenant = Tenant.objects.filter(cnic_digits=digits).first()
    if not tenant:
        return None
    ...
    values = {
        ...
        "temporary_address": tenant.temporary_address or tenant.address,
        "permanent_address": tenant.permanent_address or tenant.address,
        "address": tenant.address,
    }
    return {
        "matched": True,
        # Informational only. Submission processing re-matches by normalized CNIC
        # and never trusts this browser-visible identifier.
        "tenant_id": tenant.pk,
        "fields": [
...
def _cnic_identity_ocr_response(request, *, can_overwrite=False):
    ...
    phase = (request.POST.get("ocr_phase") or "").strip().lower()
    if phase == "front":
        ...
        result = extract_cnic_front_identity(
            request.FILES.get("cnic_front"),
            get_whatsapp_ai_config().model,
        )
        payload, status = _cnic_identity_result_payload(
            result, can_overwrite=can_overwrite, include_existing=False
        )
        if payload.get("ok"):
            cnic_value = next(
            ...
            payload.update(
                phase="front",
                cnic_verified=False,
                front_token=signing.dumps(
                    {
                        "cnic": cnic_value,
                        "scope": _cnic_staged_scope(request),
                    },
                    salt=CNIC_STAGED_TOKEN_SALT,
                    compress=True,
                ),
            )
    elif phase == "back":
        try:
            token_data = signing.loads(
                request.POST.get("front_token") or "",
                salt=CNIC_STAGED_TOKEN_SALT,
                max_age=CNIC_STAGED_TOKEN_MAX_AGE,
            )
        except SignatureExpired:
            return JsonResponse(
                {
        ...
        payload, status = _cnic_identity_result_payload(
            result, can_overwrite=can_overwrite
        )
        if payload.get("ok"):
            payload["phase"] = "back"
    else:
...
def _cnic_staged_scope(request):
    user_id = request.user.pk if request.user.is_authenticated else "public"
    value = f"{request.path}|{user_id}"
...
def _cnic_identity_payload(front_file, back_file, *, can_overwrite=False):
    ...
    return _cnic_identity_result_payload(result, can_overwrite=can_overwrite)


def _cnic_identity_result_payload(
    result, *, can_overwrite=False, include_existing=True
):
...
class TenantRegistrationSubmissionDetailView(LoginRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["can_create_registration_lease"] = (
            ...
            and self.request.user.has_perm("leases.view_lease")
            and self.request.user.has_perm("leases.change_lease")
        )
        context["properties"] = Property.objects.order_by("property_name")
        context["units"] = Unit.objects.select_related("property").order_by(
            "property__property_name", "unit_number"
        )
        approval_error = self.request.session.pop(
            f"registration_approval_error_{self.object.pk}", None
        )
        context["approval_error"] = approval_error
        context["selected_registration_property_id"] = (
            approval_error.get("property_id") if approval_error else None
        )
...
def tenant_registration_submission_review(request, pk):
    ...
    approval_actions = {"approve_registration", "approve_and_create_lease"}
    if action in approval_actions and submission.created_lease_id:
        messages.info(request, "This registration already has a lease.")
        return redirect("leases:lease_detail", pk=submission.created_lease_id)
    if action in approval_actions and submission.status == submission.STATUS_APPROVED:
    ...
    from tenants.services.registration_workflow import (
        ...
        process_registration_people,
        registration_required_party_errors,
    )
    ...
    if create_lease:
        if not (
            ...
            and request.user.has_perm("leases.view_lease")
            and request.user.has_perm("leases.change_lease")
        ):
            raise PermissionDenied
        unit = (
        ...
        if active_lease and move_in_date <= active_lease.end_date:
            ...
            request.session[f"registration_approval_error_{submission.pk}"] = {
                "message": (
                    f"{unit.property.property_name} - Unit {unit.unit_number} already has "
                    f"active Lease #{active_lease.pk} for {active_lease.tenant}, from "
                    f"{active_lease.start_date:%Y-%m-%d} through {active_lease.end_date:%Y-%m-%d}. "
                    f"Choose a start date on or after {suggested_start_date:%Y-%m-%d}, "
    ...
    party_errors = registration_required_party_errors(submission)
    if party_errors:
        messages.error(
            request,
            "Complete the required registration parties before approval: "
            + "; ".join(party_errors),
        )
        return redirect("tenants:registration_submission_detail", pk=submission.pk)
    ...
    for row in comparison:
        ...
        if decision == "update_submitted" and row.get("can_update_submitted"):
            ...
            decisions[row["field"]] = "accept_submitted"
        elif decision in {"keep_existing", "accept_submitted"}:
    ...
    if updated_values:
        candidate_data = dict(submission.submitted_data or {})
        candidate_data.update(updated_values)
        candidate_form = TenantPublicRegistrationForm(candidate_data)
        if not candidate_form.is_valid():
            messages.error(
                request,
                "Could not update the submitted value: "
                + " ".join(candidate_form.errors.get_json_data().keys()),
            )
            return redirect("tenants:registration_submission_detail", pk=submission.pk)
        ...
        submission.submitted_data = candidate_data
        submission.save(update_fields=["submitted_data"])
        TenantRegistrationSubmissionAudit.objects.create(
            submission=submission,
            edited_by=request.user,
            action="inline_submission_update",
            changes={
                field_name: {
                    "old": previous_updated_values[field_name],
                    "new": candidate_data[field_name],
                }
                for field_name in updated_values
            },
        )
        ...
        with transaction.atomic():
            ...
            if create_lease:
                ...
                lease = Lease.objects.create(
                    ...
                    water_charges=unit.water_charges,
                    internet_charges=unit.internet_charges,
                    security_deposit=unit.security_deposit_amount,
                    status="active",
                ...
                workflow_result = {
                    "people": process_registration_people(
                        submission, missing_files=missing_file_warnings
                    ),
                    "vehicles": 0,
                }
    ...
    TenantRegistrationSubmissionAudit.objects.create(
        ...
        changes={
            ...
            "lease_id": {"old": None, "new": lease.pk if lease else None},
            "submitted_updates": {
                "old": previous_updated_values,
                "new": updated_values,
            },
        },
    )
    ...
    messages.success(
        request,
        f"Registration approved. Tenant and {len(workflow_result['people'])} related people "
        "were created or updated. Vehicles remain pending until a lease is created.",
    )
...
def tenant_registration_submission_edit(request, pk):
    if not request.user.has_perm("tenants.change_tenantregistrationsubmission"):
        raise PermissionDenied
    submission = get_object_or_404(
        TenantRegistrationSubmission.objects.select_for_update().prefetch_related(
            "pending_people", "pending_vehicle_submissions"
        ),
        pk=pk,
    )
    if not submission.is_editable:
    ...
    existing_roles = {person.role for person in people}
    ...
    existing_family_count = sum(
        person.role == PendingRegistrationPerson.ROLE_FAMILY and person.pk
        for person in people
    )
    try:
        requested_family_count = int(
            (submission.submitted_data or {}).get("number_of_family_member") or 0
        )
    except (TypeError, ValueError):
        requested_family_count = 0
    blank_family_count = max(1, min(10, requested_family_count) - existing_family_count)
    for index in range(blank_family_count):
        person = PendingRegistrationPerson(
            submission=submission,
            role=PendingRegistrationPerson.ROLE_FAMILY,
        )
        person.edit_prefix = f"new-family-{index}"
        person.is_staff_entry = True
        people.append(person)

    for vehicle in vehicles:
        vehicle.edit_prefix = f"vehicle-{vehicle.pk}"
    blank_vehicle = PendingLeaseVehicleSubmission(
        pending_tenant_submission=submission,
        tenant=submission.tenant,
    )
    blank_vehicle.edit_prefix = "new-vehicle-0"
    blank_vehicle.is_staff_entry = True
    ...
    def selected_relationship(person):
        ...
        person.custom_relationship = "" if selected else person.relationship
            ...
            try:
                with transaction.atomic():
                    ...
                    for person in people:
                        ...
                        relationship_choice = (
                            request.POST.get(f"{prefix}-relationship_type") or ""
                        ).strip()
                        custom_relationship = (
                            request.POST.get(f"{prefix}-relationship_new") or ""
                        ).strip()
                        has_files = any(
