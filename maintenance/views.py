...
from .public_links import (
    get_confirmation_request_id,
    get_public_maintenance_lease,
    make_confirmation_token,
)

MAINTENANCE_SHARE_SALT = "maintenance-request-media-share"
MAINTENANCE_SHARE_MAX_AGE = 3 * 24 * 60 * 60
PUBLIC_UPLOAD_EXTENSIONS = {"jpg", "jpeg", "png", "webp", "pdf", "mp4", "mov", "webm"}
...
def _tenant_first_name(tenant):
    return getattr(tenant, "first_name", "") or _tenant_name(tenant) or str(tenant or "")
...
def _source_label(value):
    return dict(MaintenanceRequest.SOURCE_CHOICES).get(value or "", "Manual")
...
def _request_json(request, item):
    ...
    return {
        ...
        "source": item.source,
        "source_display": _source_label(item.source),
        "cost": str(item.cost),
...
def _lease_label(lease):
    tenant = lease.tenant
    unit_label = getattr(lease.unit, "unit_number", "") or str(lease.unit)
    return f"{_tenant_first_name(tenant)} - {unit_label}"


def _unit_label(unit, active_lease=None):
    unit_label = getattr(unit, "unit_number", "") or str(unit)
    tenant_label = _tenant_first_name(active_lease.tenant) if active_lease and getattr(active_lease, "tenant", None) else "Vacant"
    return f"{unit_label} - {tenant_label}"
...
def _current_leases_qs(on_date=None):
    on_date = on_date or timezone.localdate()
    return _active_leases_qs().filter(start_date__lte=on_date, end_date__gte=on_date)
...
def _unit_options(property_id=None):
    ...
    active_by_unit = {}
    for lease in _current_leases_qs().filter(unit_id__in=units.values("id")).order_by("unit_id", "-start_date", "-id"):
...
def _lease_options(property_id=None, unit_id=None):
    leases = _current_leases_qs().order_by("tenant__first_name", "tenant__last_name", "unit__unit_number")
    if property_id:
    ...
    return [
        {
            ...
            "tenant_first_name": _tenant_first_name(lease.tenant),
        }
...
def _public_submission_key(request):
    submitted = request.session.setdefault("public_maintenance_submissions", {})
    request.session.modified = True
    key = uuid.uuid4().hex
    return key, submitted


def _public_attachment_name(lease, upload, index):
    ext = os.path.splitext(upload.name or "")[1].lower()
    prop = slugify(getattr(lease.unit.property, "property_name", "") or "property") or "property"
    unit = slugify(getattr(lease.unit, "unit_number", "") or "unit") or "unit"
    date_part = timezone.localdate().strftime("%Y%m%d")
    return f"{prop}_{unit}_{date_part}_{index:03d}{ext}"


def _save_public_uploads(request, request_obj, lease):
    uploaded = []
    skipped = []
    for index, upload in enumerate(request.FILES.getlist("files"), start=1):
        ext = _safe_extension(upload.name)
        if ext not in PUBLIC_UPLOAD_EXTENSIONS:
            skipped.append(upload.name)
            continue
        original_name = upload.name
        upload.name = _public_attachment_name(lease, upload, index)
        media = MaintenanceRequestMedia.objects.create(
            request=request_obj,
            file=upload,
...
def _notify_public_request_created(request_obj):
    try:
        from notifications.models import Notification
    except Exception:
        return
    tenant = request_obj.lease_tenant
    if not tenant:
        return
    subject = f"Public maintenance request #{request_obj.pk}"
    message = (
        "A tenant submitted a maintenance request from the public link.\n\n"
        f"Request ID: #{request_obj.pk}\n"
        f"Tenant: {_tenant_name(tenant) or '-'}\n"
        f"Phone: {_tenant_phone(tenant) or '-'}\n"
        f"Property: {request_obj.building or '-'}\n"
        f"Unit: {request_obj.unit or '-'}\n"
        f"Title: {request_obj.title}\n"
        f"Attachments: {request_obj.media.filter(is_active=True).count()}"
    )
...
def _fill_request_relationships(request_obj):
    ...
    if lease:
        ...
        return request_obj
    if request_obj.unit_id:
        lease = (
            _current_leases_qs()
            .filter(unit_id=request_obj.unit_id)
            .order_by("-start_date", "-id")
            .first()
        )
        if lease:
            request_obj.lease = lease
            request_obj.tenant = lease.tenant
    return request_obj
...
class MaintenanceRequestCreateView(LoginRequiredMixin, CreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx["buildings"] = Property.objects.order_by("property_name")
        ctx["leases"] = _current_leases_qs().order_by("tenant__first_name", "tenant__last_name", "unit__unit_number")[:300]
        ctx["selected_property_id"] = ""
        ctx["selected_lease_id"] = ""
        return ctx
...
class MaintenanceRequestUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def get_context_data(self, **kwargs):
        ctx = super().get_context_data(**kwargs)
        selected_unit = getattr(self.object, "unit", None)
        selected_lease = self.object.lease
        ctx["buildings"] = Property.objects.order_by("property_name")
        ctx["leases"] = _current_leases_qs().order_by("tenant__first_name", "tenant__last_name", "unit__unit_number")[:300]
...
def request_quick_add_related(request):
    ...
    return JsonResponse({
        ...
        "tenant_name": _tenant_name(current_lease.tenant) if current_lease else "Vacant",
        "tenant_first_name": _tenant_first_name(current_lease.tenant) if current_lease else "Vacant",
...
def public_request_create(request, token):
    try:
        lease = get_public_maintenance_lease(token)
    except signing.BadSignature:
        return render(request, "maintenance/public_request_form.html", {"invalid_link": True}, status=404)

    if request.method == "POST":
        submission_key = request.POST.get("submission_key") or uuid.uuid4().hex
        submitted = request.session.setdefault("public_maintenance_submissions", {})
        if submission_key in submitted:
            existing = MaintenanceRequest.objects.filter(pk=submitted[submission_key]).first()
            if existing:
                return redirect("maintenance:public_request_confirmation", token=make_confirmation_token(existing))
        ...
        if not title:
            return render(
                request,
                "maintenance/public_request_form.html",
                {
                    "lease": lease,
                    "tenant_phone": _tenant_phone(lease.tenant),
                    "submission_key": submission_key,
                    "form_error": "Request title is required.",
                },
                status=400,
            )
        ...
        request_obj = MaintenanceRequest.objects.create(
            ...
            source=MaintenanceRequest.SOURCE_PUBLIC_LINK,
            status="new",
            priority="normal",
        )
        MaintenanceRequestStatusLog.objects.create(
            ...
            notes="Request created from public maintenance link.",
        )
        _save_public_uploads(request, request_obj, lease)
        submitted[submission_key] = request_obj.pk
        request.session["public_maintenance_submissions"] = submitted
        request.session.modified = True
        _notify_public_request_created(request_obj)
        return redirect("maintenance:public_request_confirmation", token=make_confirmation_token(request_obj))

    submission_key, _submitted = _public_submission_key(request)
    return render(
        request,
        "maintenance/public_request_form.html",
        {
...
def _public_confirmation_request(token):
    request_id = get_confirmation_request_id(token)
    return get_object_or_404(
        MaintenanceRequest.objects.select_related("tenant", "lease", "unit", "unit__property").prefetch_related("media"),
        pk=request_id,
        source=MaintenanceRequest.SOURCE_PUBLIC_LINK,
    )


def public_request_confirmation(request, token):
    try:
        item = _public_confirmation_request(token)
    except signing.BadSignature:
        return render(request, "maintenance/public_request_confirmation.html", {"invalid_link": True}, status=404)
    return render(
        request,
        "maintenance/public_request_confirmation.html",
        {
            "request_obj": item,
            "attachment_count": item.media.filter(is_active=True).count(),
            "token": token,
        },
    )


def public_request_confirmation_download(request, token):
    try:
        item = _public_confirmation_request(token)
    except signing.BadSignature:
        return HttpResponseBadRequest("Invalid confirmation link")
    attachment_count = item.media.filter(is_active=True).count()
    submitted = timezone.localtime(item.created_at).strftime("%Y-%m-%d %H:%M")
    tenant = item.lease_tenant
    lines = [
        "Maintenance Request Confirmation",
        "",
        f"Request ID: #{item.pk}",
        f"Lease tenant: {_tenant_name(tenant) or '-'}",
        f"Property: {item.building or '-'}",
        f"Unit: {item.unit or '-'}",
        f"Submitted: {submitted}",
        f"Title: {item.title}",
        f"Description: {item.description or '-'}",
        f"Uploaded attachment count: {attachment_count}",
    ]
    response = HttpResponse("\n".join(lines), content_type="text/plain; charset=utf-8")
    response["Content-Disposition"] = f'attachment; filename="maintenance-request-{item.pk}-confirmation.txt"'
    return response
