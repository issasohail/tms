...
class InvoiceListView(SingleTableView):
    ...
    def get_queryset(self):
        ...
        # Filters aligned with payment_list.html
        request = self.request
        tenant = request.GET.get("tenant") or request.GET.get("tenant_id")
        prop = request.GET.get("property") or request.GET.get("property_id")
        ...
        # Always sort by Serial# (invoice_number)
        return qs.order_by("invoice_number")

    def get_table(self, **kwargs):
        table = super().get_table(**kwargs)
        # Enforce Serial# sorting always
    ...
    def get_context_data(self, **kwargs):
        ...
        #   - tenant_options: "Tenant Name — Unit Name"
        from django.apps import apps  # local import to avoid changing your global imports
        ...
        try:
            ...
            context["property_options"] = [
                {"id": p.id, "name": p.name} for p in properties]
        except Exception:
            context["property_options"] = []
        ...
        for L in leases:
            ...
            tname = getattr(t, "name", None) or (
                f"{(getattr(t, 'first_name', '') or '').strip()} {(getattr(t, 'last_name', '') or '').strip()}".strip()
            )
            uname = getattr(u, "name", None) or getattr(
                u, "unit_number", None) or getattr(u, "code", None) or ""

            label = (tname or "Tenant")
            if uname:
                label = f"{label} — {uname}"

            opts.append({"id": tid, "label": label})
            seen.add(tid)

        context["tenant_options"] = opts
        return context
