...
class UnitInventoryItemColumn(tables.Column):
    def __init__(self, item, **kwargs):
        ...
        super().__init__(
            ...
            attrs={
                "td": {
                    "class": "text-center unit-col inventory-item-col",
                    "data-label": item.name,
                },
                "th": {
                    "class": "text-center unit-col inventory-item-col rotate-col"
                },
            },
            **kwargs,
        )

    def _effective_value(self, record):
        quantity = self.item.default_quantity
        is_included = self.item.is_active
        for row in getattr(record.property, "unit_list_inventory", []):
            if row.item_id == self.item.pk:
        ...
        for row in getattr(record, "unit_list_inventory", []):
            if row.item_id == self.item.pk:
                ...
                is_included = row.is_included
                break
        return quantity if is_included else None

    def render(self, record):
        quantity = self._effective_value(record)
        manage_url = reverse("leases:inventory_manage", args=["unit", record.pk])
        return format_html(
            '<a class="inventory-quantity-link" href="{}" '
            'title="Manage {} inventory">{}</a>',
            manage_url,
            self.item.name,
...
class UnitTable(ExportableTable):
    ...
    def __init__(self, *args, **kwargs):
        ...
        inventory_columns = [
            (f"inventory_{item.pk}", UnitInventoryItemColumn(item))
            for item in self.inventory_definitions
        ]
        kwargs["extra_columns"] = [
            *list(kwargs.pop("extra_columns", [])),
            *inventory_columns,
        ]
        kwargs["sequence"] = (
            "sn",
            ...
            *(name for name, _column in inventory_columns),
            "status",
            "is_smart_meter",
            "show_publicly",
            "actions",
    ...
    def render_property(self, value):
        return format_html(
            '<span class="property-name-compact" title="{}">{}</span>',
            value,
            value,
        )
