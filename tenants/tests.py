...
class CnicPdfUploadTests(SimpleTestCase):
    @staticmethod
    def pdf_upload(name="cnic.pdf"):
        import fitz

        document = fitz.open()
        page = document.new_page(width=420, height=260)
        page.insert_text((40, 80), "CNIC test page", fontsize=24)
        payload = document.tobytes()
        document.close()
        return SimpleUploadedFile(name, payload, content_type="application/pdf")

    def test_pdf_service_converts_first_page_to_jpeg(self):
        from PIL import Image
        from tenants.services.cnic_uploads import cnic_pdf_first_page_to_jpeg

        converted = cnic_pdf_first_page_to_jpeg(self.pdf_upload())

        self.assertEqual(converted.content_type, "image/jpeg")
        self.assertTrue(converted.name.endswith("-page-1.jpg"))
        with Image.open(converted) as image:
            self.assertEqual(image.format, "JPEG")
            self.assertGreater(image.width, image.height)

    def test_pdf_conversion_endpoint_returns_jpeg(self):
        response = self.client.post(
            reverse("tenants:cnic_pdf_to_jpeg"),
    ...
    def test_pdf_conversion_endpoint_rejects_invalid_pdf(self):
        response = self.client.post(
            reverse("tenants:cnic_pdf_to_jpeg"),
            {
                "file": SimpleUploadedFile(
                    "cnic.pdf", b"not a pdf", content_type="application/pdf"
                )
            },
        )
...
class RegistrationOnboardingTests(TestCase):
    ...
    def test_registration_approval_uses_physical_move_in_and_aligned_billing_start(self):
        ...
        submission = TenantRegistrationSubmission.objects.create(
            tenant=self.make_shell(),
            submitted_data=self.public_post_data(cnic="3520212345699"),
        )
        self.make_required_parties(submission)
        prop, unit = self.make_unit("9")
        unit.monthly_rent = Decimal("30000.00")
        unit.society_maintenance = Decimal("0.00")
        unit.water_charges = Decimal("0.00")
        unit.internet_charges = Decimal("0.00")
        ...
        data = {
            "action": "approve_and_create_lease",
            "property": prop.pk,
            "unit": unit.pk,
            "move_in_date": "2026-09-12",
            "align_billing_to_month_start": "1",
