...
from typing import Optional  # (already present above; keep it)
...
from smart_meter.models import Meter, LiveReading, MeterReading, UnknownMeter
...
LOG_PATH = str(Path(LOG_DIR) / "meter_listener.log")
...
# Avoid adding duplicate handlers if the command is restarted/imported
if not any(
    isinstance(h, ConcurrentRotatingFileHandler) and getattr(
        h, "baseFilename", None) == LOG_PATH
...
LOG_FILE_FRAMES = Path(settings.LOG_DIR) / "meter_raw_frames.log"
log = logging.getLogger("smart_meter.meter_listener")
...
KA_IDLE = 600      # start keepalive probes after 60s idle
    ...
    try:
        ...
        if op == "list":
            ...
            if not meter or not frame_hex:
                ...
                if not h:
                    resp = {"ok": False, "error": f"meter {meter} not connected"}
                else:
                    waiter: "queue.Queue" = queue.Queue()
                    _push_waiter(meter, waiter, expect_di)

                    action_guess = _decode_switch_action_from_hex(frame_hex)
                    ...
                    if action_guess in ("ON", "OFF") and not (allow_switch and initiated_by):
                        ...
                        if action_guess in ("ON", "OFF"):
                            logger.warning("!!! SWITCH_SEND peer=%s meter=%s action=%s by=%s reason=%s frame=%s",
                        ...
                        ttl = float(timeout)
                        h.enqueue_send(bytes.fromhex(frame_hex),
                                       expire_at=time.time() + ttl)

                        try:
                            reply = waiter.get(timeout=timeout)
                            resp = {"ok": True, "reply": reply.hex().upper()}
                            logger.info("CONTROL_RESULT meter=%s action=%s ok=True reply_len=%s",
                                        meter, action_guess or "UNKNOWN",
                                        len(reply) if isinstance(reply, (bytes, bytearray)) else "?")
                        except queue.Empty:
                            logger.warning("CONTROL_RESULT meter=%s action=%s ok=False error=timeout",
                                           meter, action_guess or "UNKNOWN")
                            resp = {"ok": False,
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument("--accept-bad-checksum", dest="accept_bad_checksum", action="store_true",
    ...
    def handle(self, *args, **opts):
        ...
        logging.basicConfig(
            ...
            datefmt="%Y-%m-%d %H:%M:%S",
            # force=True,  # uncomment if something else already configured logging
        )

        logger = logging.getLogger("dlt645")
