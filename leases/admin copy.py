...
from leases.models import LeaseAgreementClause, LeaseTemplate
...
class LeaseAdmin(admin.ModelAdmin):
    ...
    def get_form(self, request, obj=None, **kwargs):
        ...
        if 'notes' in form.base_fields:  # Replace 'notes' with your field name
        ...
        # if 'another_field' in form.base_fields:
        #     form.base_fields['another_field'].widget = forms.Textarea(attrs={'rows': 10, 'cols': 80})
    ...
    def security_deposit_display(self, obj):
        return f"${obj.security_deposit:,.2f}" if obj.security_deposit else '-'
    ...
    actions = ['download_lease_agreement']
...
class LeaseAdmin(admin.ModelAdmin):
    actions = [return_security_deposit]
...
class LeaseAgreementClauseAdmin(admin.ModelAdmin):
    actions = [bulk_update_placeholder, apply_template]
...
class ClauseInline(admin.TabularInline):
    model = LeaseAgreementClause
    extra = 0
    readonly_fields = ('is_customized', 'created_at', 'updated_at')
    fields = ('clause_number', 'template_text',
