...
class LeaseForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.instance.pk:
            ...
            existing_proration = self.instance.invoices.filter(
                description__startswith="[MOVE_IN_PRORATION]"
            ).exclude(
                status="cancelled"
            ).exclude(
                lifecycle_status__in={"cancelled", "void"}
            ).order_by("id").first()
            from leases.utils.billing import stored_move_in_proration_options

            stored_mode, stored_days = stored_move_in_proration_options(
                existing_proration
            )
            self.fields["create_move_in_proration"].initial = bool(
                existing_proration
            )
            self.fields["move_in_proration_mode"].initial = stored_mode
            self.fields["move_in_proration_days"].initial = stored_days
...
class LeaseInspectionCreateForm(forms.ModelForm):
    default_status = forms.ModelChoiceField(
        ...
        required=False,
        label="Default status for copied rows",
        widget=forms.Select(attrs={"class": "form-select form-select-sm"}),
        help_text="Optional. Applies this status to all template rows when the sheet is created.",
    )
    ...
    class Meta:
        ...
        widgets = {
            ...
            "inspector_name": forms.TextInput(attrs={"class": "form-control form-control-sm", "placeholder": "Type inspector name"}),
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["default_status"].queryset = InspectionStatus.objects.filter(active=True)
        good_status = InspectionStatus.objects.filter(name__iexact="Good", active=True).first()
        if good_status:
            self.fields["default_status"].initial = good_status
...
class InspectionPhotoForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            "image": forms.ClearableFileInput(attrs={"class": "form-control form-control-sm", "accept": "image/*,video/*"}),
...
class AgreementSignatureTemplateForm(forms.ModelForm):
    class Meta:
        ...
        fields = [
            ...
            "show_agreement_page_numbers",
            "legal_first_page_top_reserve", "letter_first_page_top_reserve",
            "legal_qr_reserve_width",
            "legal_qr_reserve_height",
            "letter_qr_reserve_width", "letter_qr_reserve_height",
            "legal_identity_bottom_reserve",
        ...
        widgets = {
            ...
            "letter_first_page_top_reserve": forms.NumberInput(attrs={"class": "form-control", "step": "0.1"}),
            ...
            "letter_qr_reserve_width": forms.NumberInput(attrs={"class": "form-control", "step": "0.1"}),
            "letter_qr_reserve_height": forms.NumberInput(attrs={"class": "form-control", "step": "0.1"}),
