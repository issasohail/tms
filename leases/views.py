...
from .forms import LeaseForm, LeaseFamilyFormSet, LeaseRenewalInlineFormSet
...
from .models import Lease, LeaseFamily, DefaultClause
...
class LeaseDetailView(LoginRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx['original_period_end'] = (
            ...
            if first_renewal else lease.end_date
        )
...
def edit_clauses(request, pk):
    ...
    if False and (not lease.generated_agreement_pdf or not os.path.exists(lease.generated_agreement_pdf.path)):
        context = {
            'lease': lease,
...
def download_preview_pdf(request, lease_id):
    ...
    history = (
        ...
        if history_id else latest_history(lease)
    )
    copy_previous_history_clauses(lease, history)
    clauses = history.clauses.all().order_by('clause_number')
    lease_for_preview = copy(lease)
    for field in [
        "start_date", "end_date", "agreement_date", "monthly_rent",
        "society_maintenance", "water_charges", "internet_charges",
        "agreement_charges", "security_deposit", "witness1_name",
        "witness1_cnic", "witness2_name", "witness2_cnic", "terms",
        "rent_increase_percent",
    ]:
        if hasattr(history, field):
            setattr(lease_for_preview, field, getattr(history, field))
...
class SecurityDepositDeleteView(LeaseSecurityMixin, DeleteView):
    ...
    def post(self, request, *args, **kwargs):
        if request.POST.get('confirm_delete') != 'yes':
            messages.error(self.request, "Please confirm before deleting the security deposit transaction.")
            return redirect(self.get_success_url())
...
from .models import AgreementPlaceholder, Lease, DefaultClause, WhatsAppTemplate
from .forms import AgreementPlaceholderForm, DefaultClauseForm, LeaseClauseFormSet, WhatsAppTemplateForm
...
def agreement_placeholder_list(request):
    placeholders = AgreementPlaceholder.objects.order_by("category", "sort_order", "key")
    active_count = placeholders.filter(is_active=True).count()
    return render(request, "leases/agreement_placeholder_list.html", {
...
def agreement_placeholder_create(request):
    ...
    return render(request, "leases/agreement_placeholder_form.html", {
        "form": form,
...
def lease_welcome_whatsapp(request, pk):
    ...
    return render(request, "leases/lease_whatsapp_preview.html", {
        "lease": lease,
        "template": template,
...
def whatsapp_template_list(request):
    ...
    return render(request, "leases/whatsapp_template_list.html", {
        "templates": templates,
    })
...
def whatsapp_template_create(request):
    ...
    return render(request, "leases/whatsapp_template_form.html", {
        "form": form,
...
def download_preview_docx(request, lease_id):
    ...
    for clause in clauses:
        clause.rendered_text = do_replace_placeholders(clause.template_text, lease_for_preview)
...
def reset_clauses_from_default(request, pk):
    ...
    default_clauses = DefaultClause.objects.filter(is_active=True).order_by("clause_number")

    if not default_clauses.exists():
        messages.error(request, "No active default clauses found.")
        return redirect(f"{reverse('leases:edit_clauses', kwargs={'pk': pk})}?history={history.pk}")
