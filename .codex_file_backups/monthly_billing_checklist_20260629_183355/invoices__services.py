...
def generate_monthly_billing_electric(run, *, dry_run=False):
    ...
    for item in run.items.filter(electric_required=True).exclude(status__in=[MonthlyBillingRunItem.STATUS_PENDING, MonthlyBillingRunItem.STATUS_EXCLUDED]).select_related("lease"):
        installations = MeterInstallation.objects.filter(
            lease=item.lease,
            meter__meter_type="electric",
            meter__billing_mode="postpaid",
...
def prepare_monthly_billing_ready(run):
    for item in run.items.exclude(status__in=[
...
def generate_monthly_billing_pdfs(run):
    ...
    for item in run.items.filter(status=MonthlyBillingRunItem.STATUS_READY).exclude(status=MonthlyBillingRunItem.STATUS_EXCLUDED).select_related("invoice"):
...
def run_monthly_billing_full(run, *, created_by=None):
    _run_log(run, "run billing started")
    run_monthly_billing_preflight(run.billing_month, created_by=created_by)
...
def rollback_monthly_billing_run(run, *, user=None):
    blocked = []
    for item in run.items.exclude(status__in=[MonthlyBillingRunItem.STATUS_ROLLED_BACK, MonthlyBillingRunItem.STATUS_EXCLUDED]):
        try:
