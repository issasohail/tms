...
class PaymentCreateView(LoginRequiredMixin, CreateView):
    ...
    def get_lease(self):
        # Accept lease from POST first (submit), then GET (?lease or ?lease_id)
        lease_id = (
            self.request.POST.get('lease') or
            self.request.GET.get('lease') or
            self.request.GET.get('lease_id')
        )
        if lease_id:
    ...
    def get_form_kwargs(self):
        ...
        # Always pass the resolved lease into the form so it can widen the queryset
        lease = self.get_lease()
        if lease:
            kwargs['lease'] = lease
        return kwargs
    ...
    def get_form(self, form_class=None):
        ...
        include_inactive = str(
            ...
            self.request.POST.get('include_inactive', '')
        ...
        if not include_inactive:
            if selected_lease_id:
                # Active leases + the one we’re working with (even if inactive)
                lease_qs = lease_qs.filter(
                    Q(status='active') | Q(pk=selected_lease_id))
            else:
                lease_qs = lease_qs.filter(status='active')

        form.fields['lease'].queryset = lease_qs

        # Preselect for GET render (and keep selection on error re-render)
        if selected_lease_id:
            try:
                form.fields['lease'].initial = lease_qs.model.objects.only(
                    'id').get(pk=selected_lease_id).pk
    ...
    def form_valid(self, form):
        """
        Safety net: if the ModelForm didn't bind the lease (shouldn't happen now),
        bind it here so saving succeeds.
        """
    ...
    def get_context_data(self, **kwargs):
        ...
        # PaymentCreateView.get_context_data (add near where recent_payments is set)
        size = 10
        try:
        ...
        context['recent_size_options'] = [10, 20, 50]  # <-- add this
        context['recent_payments'] = Payment.objects.all().order_by('-id')[:size]

        # PaymentCreateView.get_context_data
        include_inactive = self.request.GET.get('include_inactive') == 'on'
        if include_inactive:
            context['active_tenants'] = Tenant.objects.all(
        ...
        # If lease is provided in URL, only show that lease
        lease_id = self.request.GET.get('lease')
        if lease_id:
            context['leases'] = Lease.objects.filter(pk=lease_id).select_related(
                'tenant', 'unit', 'unit__property'
            )
        else:
            context['leases'] = Lease.objects.filter(status='active').select_related(
        ...
        posted_lease_id = self.request.POST.get('lease')
        if posted_lease_id:
            context['leases'] = Lease.objects.filter(pk=posted_lease_id).select_related(
                'tenant', 'unit', 'unit__property'
            )
        return context
...
def get_filtered_leases(request):
    ...
    for l in qs.order_by('tenant__first_name', 'tenant__last_name'):
        ...
        text = f"{tenant_short} | {prop_short}-{unit_no} | Bal: {bal_fmt}"
