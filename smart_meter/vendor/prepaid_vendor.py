...
class DLT645_2007_Prepaid:
    ...
    def build_frame(self, meter_address: str, parameters: Dict[str, Any]) -> bytes:
        ...
        param_structure = [
            ...
            ('contract_amount_limit', 4, 'int'),     # Contract amount limit (fen)
            ('max_load_power_limit', 3, 'int'),      # Max load power limit (W)
            ('load_power_delay', 1, 'int'),          # Load power delay (s)
            ('rate1_price_1', 4, 'price'),           # First set rate 1 price (usd/kWh)
            ('rate1_price_2', 4, 'price'),           # First set rate 2 price (usd/kWh)
            ('rate1_price_3', 4, 'price'),           # First set rate 3 price (usd/kWh)
            ('rate1_price_4', 4, 'price'),           # First set rate 4 price (usd/kWh)
            ('rate2_price_1', 4, 'price'),           # Second set rate 1 price (usd/kWh)
            ('rate2_price_2', 4, 'price'),           # Second set rate 2 price (usd/kWh)
            ('rate2_price_3', 4, 'price'),           # Second set rate 3 price (usd/kWh)
            ('rate2_price_4', 4, 'price'),           # Second set rate 4 price (usd/kWh)
            ...
            ('step1_price_1', 4, 'price'),           # First step price 1 (usd/kWh)
            ('step1_price_2', 4, 'price'),           # First step price 2 (usd/kWh)
            ('step1_price_3', 4, 'price'),           # First step price 3 (usd/kWh)
            ('step1_price_4', 4, 'price'),           # First step price 4 (usd/kWh)
            ('step2_value_1', 4, 'int'),             # Second step value 1 (kWh)
            ('step2_value_2', 4, 'int'),             # Second step value 2 (kWh)
            ('step2_value_3', 4, 'int'),             # Second step value 3 (kWh)
            ('step2_price_1', 4, 'price'),           # Second step price 1 (usd/kWh)
            ('step2_price_2', 4, 'price'),           # Second step price 2 (usd/kWh)
            ('step2_price_3', 4, 'price'),           # Second step price 3 (usd/kWh)
            ('step2_price_4', 4, 'price'),           # Second step price 4 (usd/kWh)
        ]
            ...
            if param_name in parameters:
                ...
                if param_type == 'price':
                    ...
                    if isinstance(value, (int, float)):
                        ...
                        raise ValueError(f"Price parameter {param_name} must be numeric")
                else:
        ...
        if data_field[:4] == self.PARAM_IDENTIFIER:
            ...
            for param_name, length, param_type in param_structure:
                if offset + length <= len(param_data):
                    ...
                    if param_type == 'price':
                        parsed_params[param_name] = self._bcd_to_float(param_bytes)
                    else:
...
if __name__ == "__main__":
    ...
    example_params = {
        ...
        'rate_switch_time': 2312312359,       # Rate switch time: 2023-12-31 23:59
        'step_switch_time': 2401010000,       # Step switch time: 2024-01-01 00:00
        ...
        'contract_amount_limit': 100000,      # Contract amount limit (1000.00 usd)
