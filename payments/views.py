...
class PaymentListView(SingleTableView):
    ...
    def get_queryset(self):
        ...
        ZERO_DB = Value(ZERO, output_field=DECIMAL)
        ...
        invoice_map = {row["lease_id"]: (row["total"] or ZERO) for row in invoice_totals}
        payment_map = {row["lease_id"]: (row["total"] or ZERO) for row in payment_totals}

        for payment in queryset:
            lease_id = payment.lease_id
            total_invoiced = invoice_map.get(lease_id, ZERO)
            total_paid = payment_map.get(lease_id, ZERO)

            # keep Decimal; if you need 2dp formatting, do it in template
    ...
    def get_context_data(self, **kwargs):
        ...
        total_amount = queryset.aggregate(
            total=Coalesce(Sum("amount"), ZERO_DB)
        )["total"]
...
from .services import rebuild_allocation  # ✅ the service function
...
class PaymentCreateView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        # If allocation exists, go to allocation detail; otherwise go to payment detail
        if alloc:
...
def send_receipt(request, payment_id):
    ...
    if request.method == 'POST':
        ...
        if request.POST.get('send_email'):
            ...
            return redirect('payments:allocation_detail', pk=payment.id)
...
def build_payment_receipt_message(request, pay):
    ...
    lines.extend([
        ...
        f"New Balance: Rs. {float(new_balance):,.2f}",
        "",
        "Thank you!",
    ])
